...
const variantsContainer = document.querySelector("#products .right");
...
let variantIndex = variantsContainer.querySelectorAll(".variant-card").length - 1;
...
addVariantBtn.addEventListener("click", async (e) => {
	...
	variantsContainer.insertBefore(div, cropperModal);
...
variantsContainer.addEventListener("change", async (e) => {
	if (e.target.classList.contains("variant-image-input")) {
		...
		variantCard
			.querySelectorAll('input[name^="variants"][name$="[existingImages][]"]')
...
variantsContainer.addEventListener("click", (e) => {
	...
	if (clickedImageWrapper && clickedImageWrapper.closest("label.preview.imageActive")) {
		...
		const imgSrc = imgElement.getAttribute("data-original-src") || imgElement.src;
		...
		const isExistingImage =
			!clickedImageWrapper.dataset.isNewImage || clickedImageWrapper.dataset.isNewImage === "false";

		if (isExistingImage) {
			const variantCard = clickedImageWrapper.closest(".variant-card");
			const hiddenInputs = variantCard.querySelectorAll(
				'input[name^="variants"][name$="[existingImages][]"]'
			);

			const hiddenInputToRemove = Array.from(hiddenInputs).find(
				(input) => input.value === imgElement.src
			);
			if (hiddenInputToRemove) {
				hiddenInputToRemove.dataset.isCropped = "true";
				hiddenInputToRemove.style.display = "none";
			}
		}
...
cropSaveBtn.addEventListener("click", () => {
	...
	if (previewImg) {
		...
		previewImg.dataset.originalSrc = croppedDataURL;
	}

	const originalFileName = previewWrapper.dataset.fileName || `cropped-existing-${Date.now()}.png`;
	...
	const isExistingImage =
		!previewWrapper.dataset.isNewImage || previewWrapper.dataset.isNewImage === "false";
	...
	if (isExistingImage && !fileFoundAndReplaced) {
		dataTransfer.items.add(croppedFile);
	}

	if (isExistingImage) {
		const variantCard = currentPreviewWrapper.closest(".variant-card");
		const hiddenInput = variantCard.querySelector(
			'input[name^="variants"][name$="[existingImages][]"][data-is-cropped="true"]'
		);
		if (hiddenInput) {
			hiddenInput.remove();
		}
	...
	previewWrapper.dataset.fileName = originalFileName;
...
cropCancelBtn.addEventListener("click", () => {
	if (currentCropper) {
		...
		const variantCard = cropperModal.querySelector('input[data-is-cropped="true"]');
		if (variantCard) {
			variantCard.style.display = "";
			delete variantCard.dataset.isCropped;
		}
	}
	cropperModal.style.display = "none";
});
...
formSubmit.addEventListener("submit", async (e) => {
	e.preventDefault();
	const id = formSubmit.getAttribute("data-id").trim();
