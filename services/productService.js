		...
		const productBasedOnCategory = await productModel.aggregate([
			...
			},
			{
				$group: {
					...
					vrId: { $first: "$allVariants._id" },
					price: { $first: "$allVariants.price" },
					images: { $first: "$allVariants.images" },
					discountedPrice: { $first: "$allVariants.discountedPrice" },
					stock: { $first: "$allVariants.stock" },
					sizes: { $first: "$allVariants.sizes" },
				},
			},
			{
				$project: {
